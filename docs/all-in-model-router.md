...
- **Объём:** сразу и Claude-логины, и все API-провайдеры.
  ChatGPT (`codex login`, без endpoint и ключа) добавлен позже, коммитом
  `60c9106`: роутер лениво поднимает мост `gptbridge.ChatGPTBridge` и
  отдаёт ряду его loopback-адрес и ключ. Подробности —
  `internal/allin/CLAUDE.md`.
