...
Package gotchas live beside their code: `internal/tui/CLAUDE.md` (diff pager, worktree poll, popup backdrop), `internal/attention/CLAUDE.md` (parked turn, cleared chat), `internal/gptbridge/CLAUDE.md` (the Codex bridge), `internal/rolefix/CLAUDE.md` (the Featherless proxy), `internal/usage/CLAUDE.md` (rollout accounting). They load when a file in that package is opened — read them before changing one.

Those files are **audited as production source**: `isProductionAuditTextPath`
matches any tracked path under `internal/`, so `validateShellProductionHostEffectOwnership`
...
Reword it, or keep that gotcha in this file.
